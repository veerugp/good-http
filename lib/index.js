...
class GoodHttp extends Stream.Writable {
    ...
    _sendMessages(callback) {
        ...
        Wreck.request('post', this._endpoint, wreckOptions, (err,res)=>{
            if(err){
                console.log('Error occured in good-http:' + JSON.stringify(err));
            }else{
                console.log('Good-http:Successful: ' + this._endpoint + ':' + res.statusCode)
                console.log('Good-http:Successful: ' + this._endpoint + ':' + JSON.stringify(wreckOptions.payload));
                console.log('Good-http:Successful: ' + this._endpoint + ':' + JSON.stringify(res.rawHeaders));
            }
        });
    }
}
